const {Api,API_ID,API_HASH,STATE_SECRET,SESSION_SECRET,requireConfig,seal,open,authHeader,supabase,newClient,json,err}=require("./_common");
  ...
  try{
    ...
    try{
      ...
      if(String(e?.errorMessage||e?.message||"").includes("SESSION_PASSWORD_NEEDED")){
        return json(res,200,{ok:false,needsPassword:true,authToken:seal({
