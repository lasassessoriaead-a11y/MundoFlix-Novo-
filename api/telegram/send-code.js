...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    const sent=await client.invoke(new Api.auth.SendCode({phoneNumber:phone,apiId:API_ID,apiHash:API_HASH,settings:new Api.CodeSettings({})}));
    const state={exp:Date.now()+10*60*1000,phone,phoneCodeHash:sent.phoneCodeHash,preSession:client.session.save()};
